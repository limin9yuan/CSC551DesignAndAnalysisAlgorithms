...
private:
    ...
    double       mTotalCost = 0;          //minimum total weight
...
public:
    ...
    void makeSet ( int vCount ) {
        if (vCount < 0) {
            cerr<<"Illegal count "<<vCount<<endl;
            return;
        }
        else {
            for (int i = 0; i < vCount; i++){
    ...
    bool sameSet ( int u, int v ) {
        ...
        if (u < 0 || u >= mVertexCount || v < 0 || v >= mVertexCount)
            return false;
        for (int i = 0; i < mVertexCount; i++) {
            if (mVertexSet[u][i] != mVertexSet[v][i]) {
                return false;
            }
        }
        return true;
    }
    //copy all element in v into u
    ...
    void vertexSetUnion ( int u, int v ) {
        if (u < 0 || u > mVertexCount || v < 0 || v > mVertexCount) {
            ...
            //merge u and v
            for (int i = 0; i < mVertexCount; i++) {
                if (mVertexSet[u][i] == true) {
                    mVertexSet[v][i] = true;
                }
                else if (mVertexSet[v][i] == true) {
                    mVertexSet[u][i] = true;
                }
            }
            //merge u or v into other index
            for (int i = 0; i < mVertexCount; i++) {
                if (i != u && i != v && mVertexSet[u][i] == true) {
                    copy(i, u);
                }
            }
        }
